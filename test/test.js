...
tape( 'the function replaces elements equal to `NaN`', function test( t ) {
	var expected;
	var actual;
	var alpha;
	var start;
	...
	actual = gfillNaN( [ x, alpha, start, end ] );
	t.strictEqual( actual, x, 'returns expected value' );

	expected = [ 0.0, -2.0, 3.0, 0.0, 4.0, -6.0 ];
...
tape( 'the function supports an input ndarray having a non-unit stride', function test( t ) {
	...
	expected = [ 0.0, -2.0, 3.0, NaN, 4.0, -6.0 ];
...
tape( 'the function supports an input ndarray having a non-zero offset', function test( t ) {
	...
	expected = [ NaN, -2.0, 3.0, 0.0, 4.0, -6.0 ];
	t.strictEqual( isSameArray( xbuf, expected ), true, 'returns expected value' );
...
tape( 'the function supports specifying a fill range for an input ndarray having a non-unit stride', function test( t ) {
	...
	expected = [ NaN, -2.0, 0.0, 4.0 ];
	t.strictEqual( isSameArray( xbuf, expected ), true, 'returns expected value' );

	t.end();
});

tape( 'if none of the elements are equal to `NaN`, the function returns the input ndarray unchanged', function test( t ) {
	var expected;
	var actual;
	var alpha;
	var start;
